 ...
 * Name: set_rectangular_parking_right
 * ID: 10
 */

class set_rectangular_parking_right: public RPCFunctionality
{
  private:
    std::string type_name_ = "set_rectangular_parking_right";
  ...
  public:
    ...
    struct Parameter : public FuncComponentBase
    {
		...
		bool set_rectangular_parking_right;
 
		bool Parameter_serialize_topic(ucdrBuffer* writer, const Parameter* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->set_rectangular_parking_right);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->set_rectangular_parking_right);
