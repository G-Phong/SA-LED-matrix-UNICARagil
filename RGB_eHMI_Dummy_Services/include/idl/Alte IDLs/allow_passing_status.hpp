 ...
 * Name: allow_passing_status
 * ID: 10
 */

class allow_passing_status: public Functionality
{
  private:
    std::string type_name_ = "allow_passing_status";
  ...
  public:
    ...
    struct Data : public FuncComponentBase
    {
		...
		bool allow_passing;
		float distance_passing_vehicle;
		float distance_narrowing;
		bool allow_pedestrian_crossing;
		float time_distance_pedestrian;
 
		bool Data_serialize_topic(ucdrBuffer* writer, const Data* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->allow_passing);
		
		    (void) ucdr_serialize_float(writer, topic->distance_passing_vehicle);
		
		    (void) ucdr_serialize_float(writer, topic->distance_narrowing);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->allow_passing);
		
		    (void) ucdr_deserialize_float(reader, &topic->distance_passing_vehicle);
		
		    (void) ucdr_deserialize_float(reader, &topic->distance_narrowing);
