 ...
 * Name: set_turning_right_status
 * ID: 10
 */

class set_turning_right_status: public RPCFunctionality
{
  private:
    std::string type_name_ = "set_turning_right_status";
  ...
  public:
    ...
    struct Parameter : public FuncComponentBase
    {
		...
		bool set_turning_right_status;
 
		bool Parameter_serialize_topic(ucdrBuffer* writer, const Parameter* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->set_turning_right_status);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->set_turning_right_status);
