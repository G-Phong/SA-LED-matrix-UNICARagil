 ...
 * Name: leaving_parking_rect_right_status
 * ID: 10
 */

class leaving_parking_rect_right_status: public Functionality
{
  private:
    std::string type_name_ = "leaving_parking_rect_right_status";
  ...
  public:
    ...
    struct Data : public FuncComponentBase
    {
		...
		bool leaving_parking_rectangular_right;
 
		bool Data_serialize_topic(ucdrBuffer* writer, const Data* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->leaving_parking_rectangular_right);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->leaving_parking_rectangular_right);
