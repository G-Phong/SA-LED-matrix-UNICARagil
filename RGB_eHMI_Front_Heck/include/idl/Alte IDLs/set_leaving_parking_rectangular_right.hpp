 ...
 * Name: set_leaving_parking_rectangular_right
 * ID: 10
 */

class set_leaving_parking_rectangular_right: public RPCFunctionality
{
  private:
    std::string type_name_ = "set_leaving_parking_rectangular_right";
  ...
  public:
    ...
    struct Parameter : public FuncComponentBase
    {
		...
		bool set_leaving_parking_rectangular_right;
 
		bool Parameter_serialize_topic(ucdrBuffer* writer, const Parameter* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->set_leaving_parking_rectangular_right);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->set_leaving_parking_rectangular_right);
