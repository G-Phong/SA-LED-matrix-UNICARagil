 ...
 * Name: set_leaving_parking_rect_left_status
 * ID: 10
 */

class set_leaving_parking_rect_left_status: public RPCFunctionality
{
  private:
    std::string type_name_ = "set_leaving_parking_rect_left_status";
  ...
  public:
    ...
    struct Parameter : public FuncComponentBase
    {
		...
		bool set_leaving_parking_rect_left_status;
 
		bool Parameter_serialize_topic(ucdrBuffer* writer, const Parameter* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->set_leaving_parking_rect_left_status);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->set_leaving_parking_rect_left_status);
