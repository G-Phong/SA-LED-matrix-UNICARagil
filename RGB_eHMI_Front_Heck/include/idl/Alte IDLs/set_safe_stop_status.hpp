 ...
 * Name: set_safe_stop_status
 * ID: 10
 */

class set_safe_stop_status: public RPCFunctionality
{
  private:
    std::string type_name_ = "set_safe_stop_status";
  ...
  public:
    ...
    struct Parameter : public FuncComponentBase
    {
		...
		bool set_safe_stop_status;
 
		bool Parameter_serialize_topic(ucdrBuffer* writer, const Parameter* topic)
		{
		    (void) ucdr_serialize_bool(writer, topic->set_safe_stop_status);
		...
		{
		    (void) ucdr_deserialize_bool(reader, &topic->set_safe_stop_status);
