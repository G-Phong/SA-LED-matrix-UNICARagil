 ...
 * Name: turning_right_status
 * ID: 10
 */

class turning_right_status: public Functionality
{
  private:
    std::string type_name_ = "turning_right_status";
  ...
  public:
    ...
    struct Data : public FuncComponentBase
    {
		...
		float turning_right_dynamic_module_planned_t;
 
		bool Data_serialize_topic(ucdrBuffer* writer, const Data* topic)
		{
		    (void) ucdr_serialize_float(writer, topic->turning_right_dynamic_module_planned_t);
		...
		{
		    (void) ucdr_deserialize_float(reader, &topic->turning_right_dynamic_module_planned_t);
